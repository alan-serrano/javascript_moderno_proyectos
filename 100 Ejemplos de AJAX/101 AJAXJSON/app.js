...
boton1.addEventListener('click', function insertarInformacionPersona() {
...
boton2.addEventListener('click', function insertarInformacionPersonas() {
    const xhr = new XMLHttpRequest;
    ...
    xhr.onload = function onloadFunction() {
        if(this.status === 200){
            ...
            let htmlTemplate = '';
            personal.forEach(function forEach(persona) {
                htmlTemplate += `
                    <ul>
                        <li>ID: ${persona.id}</li>
                        <li>Nombre: ${persona.nombre}</li>
